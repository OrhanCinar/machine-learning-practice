...
class PolicyEstimator():
    def __init__(self, num_outputs, reuse=False, trainable=True):
        ...
        self.states = tf.placeholder(
            shape[None, 84, 84, 4], dtype=tf.uint8, name="")
        ...
        with tf.variable_scope("shared", reuse=reuse):
            tf1 = build_shared_network(X, add_summaries=(not reuse))

        with tf.variable_scope("policy_net"):
            self.logits = tf.coontrib.layers.fully_connected(
        ...
        self.entropy = tf.reduce_sum(
            self.probs * tf.log(self.probs), 1, name="entropy")
        ...
        if trainable:
            ...
            self.grads_and_vars = [[grad, var] for grad,
        ...
        sumaries = [
            s for s in summary_ops if "policy_net" in s.name
            or "shared" in s.name]
...
class ValueEstimator:
    ...
    def __init__(self, reuse=False, trainable=True):
        self.states = tf.placeholder(shape=[None], dtype=tf.float32, name="X")
        ...
        with tf.variable_scope("value_net"):
            ...
            self.logits = tf.squeeze(self.logits, squeeze_dims=[
                1], name="logits")
    ...
    var_scope_name = tf.get_variable_scope().name
    summary_ops = tf.get_collection(tf.GroupKeys.SUMMARIES)
    sumaries = [
        s for s in summary_ops if "policy_net" in s.name or "shared" in s.name]
    sumaries = [s for s in summary_ops if var_scope_name is s.name]
    self.summaries = tf.summary.merge(sumaries)
